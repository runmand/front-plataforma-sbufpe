...
export default function Index(props: TPROPS) {
    ...
    async function getPDFOLD() {
        const payload = {
            id: localStorage.getItem("userId"),
        };

        const result = await http
            .post("/history/pdf", payload)
            .then((r) => {
    ...
    async function getPDF(type: "teoric" | "pratical") {
        const payload = {
            id: localStorage.getItem("userId"),
        };

        const result = await http
            .post(`/history/pdf/${type}`, payload)
            .then((r) => {
                const data = r.data as requestResponse;
    ...
    async function downloadPlanejaPDF(typeData: "teoric" | "pratical") {
        const { data, type } = await getPDF(typeData);
        ...
        // Set the anchor element's download attribute to the PDF file name
        a.download = `Resposta do planejaSD ${
            typeData == "pratical" ? "pratico" : "teórico"
            ...
            <Page style={stylesPDFTeorico.page}>
                ...
                <View style={stylesPDFTeorico.section}>
                    ...
                    {stepValues.secondStep.defined_problems.map((problem, index) => (
                        <View key={index}>
                            <Text style={stylesPDFTeorico.subtitle}>Problema Definido {problem.id}</Text>
                    ...
                    {stepValues.fourthStep.actions.map((action, index) => (
                        <View key={index}>
                            ...
                            {action.responsibles.map((responsible, i) => (
                                <View key={i}>
                                    <Text style={stylesPDFTeorico.text}>Responsável: {responsible.responsible}</Text>
                                    <Text style={stylesPDFTeorico.text}>Motivação: {responsible.motivation}</Text>
                                    <Text style={stylesPDFTeorico.text}>Estratégias: {responsible.strategies}</Text>
                                </View>
                            ))}
                            <Text style={stylesPDFTeorico.text}>Recursos:</Text>
                            {action.resources.map((resource, i) => (
                                <View key={i}>
                                    <Text style={stylesPDFTeorico.text}>Recurso: {resource.resource}</Text>
                                    <Text style={stylesPDFTeorico.text}>É Recurso Crítico: {resource.itsCricticalResource}</Text>
                                    <Text style={stylesPDFTeorico.text}>Estratégias Descritas: {resource.described_strategies}</Text>
                                </View>
                            ))}
                        </View>
                    ))}
    ...
    return (
        <Drawer anchor={"left"} open={props.isOpen} onClose={() => props.onClose()} style={modalStyle}>
            ...
            <button
                onClick={() => downloadPlanejaPDF("teoric")}
                style={{
                    padding: "3px",
                    height: "38px",
                    textTransform: "uppercase",
            ...
            >
                Baixar PDF - PlanejaSD Teórico
            </button>

            <button
                onClick={() => downloadPlanejaPDF("pratical")}
                style={{
                    padding: "3px",
                    height: "38px",
                    textTransform: "uppercase",
