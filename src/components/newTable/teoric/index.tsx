...
export default function Index(props: PROPS) {
  ...
  function filterTeoric(establishment: string, city: string, participant: string,mydata: boolean){    
    if (establishment == "*" && city == "*" && participant == "*" && mydata == false){      
      ...
      allFormattedDataTeoric.forEach(element => {  
        ...
        if (filterBy(element[0].answer, establishment, 'estabelecimento') && 
  ...
  useEffect(() =>{    
      ...
      props.setFilterTeoric({establishment: removeDuplicates(establishmentArray), city: removeDuplicates(cityArray), participant: removeDuplicates(participantArray)})         
  }, [praticalAnswer])
