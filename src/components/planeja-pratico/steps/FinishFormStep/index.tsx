...
export const FinishFormStep = ({ stepValues, onClickPrevStep }: IProps) => {
    ...
    async function sendData(payload: ICreatePayload) {
        try {
            const { data } = await http.post("/practical-plan-question-answer", payload);
            return data;
        } catch (err) {
            console.error(err);
            setError(true);
    ...
    async function sendCertificate() {
        try {
            await http.post("/plan-question-answer/certificate", { email: values.email, name: values.names, form: "P" });
        } catch {
            console.log("error");
        }
    }
            ...
            {sendingData ? (
                ...
                <form onSubmit={handleSendForm}>
                    <Typography fontWeight={1000} fontSize={16} marginBottom={"15px"}>
                        Parabéns!!! Você (s) tem um Plano de Ação em Saúde Bucal, incluindo as estratégias para aumentar sua governabilidade e
                        viabilidade, para intervenção sobre seu estabelecimento de saúde (ou rede de atenção). Você pode baixá-lo em PDF e utilizá-lo
                    ...
                    <Typography fontWeight={1000} fontSize={16} marginBottom={"15px"}>
                        Agradecemos sua participação, esperando que o GestBucalSD esteja promovendo governança inteligente e melhoria da qualidade dos
                        serviços de saúde bucal e fortalecimento do Sistema Único de Saúde
                    </Typography>
                    ...
                    <Paper sx={{ mt: 5, p: 4 }}>
                        <Typography fontWeight={700} fontSize={20}>
                            Indique para cada Domínio, até 2 indicadores com piores classificações. Digite-os abaixo:
                        </Typography>
                    ...
                    <Paper sx={{ mt: 5, p: 4 }}>
                        <Typography fontWeight={700} fontSize={20}>
                            Diante da técnica de priorização/hierarquização dos problemas, quais problemas foram definidos para a intervenção?
                            Digite-os abaixo:
                        </Typography>
                                ...
                                <Typography fontWeight={700} fontSize={16}>
                                    ...
                                    {stepValues.secondStep.defined_problems.map((item, index) => (
                                        <Typography key={item.id} fontWeight={700} fontSize={16}>
                                            {item.answer}
                                            {index !== stepValues.secondStep.defined_problems.length - 1 && (
                                                <>
                                                    {", "}
                                                    <br />
                                                    <br />
                    ...
                    <Paper sx={{ mt: 5, p: 4 }}>
                        ...
                        <Typography fontWeight={700} fontSize={20} mt={5}>
                            Apesar de você já ter chegado na definição do (s) problema (s) para a intervenção, na verdade o plano de ação não é para
                            intervir diretamente o sobre ele, mas sim sobre a sua causa principal, denominado nó-crítico. Por isso, é necessário
                            explicar o problema. Obs: caso você/equipe tenha definido mais de um problema à intervenção de domínios ou módulos
                            operacionais diferentes, talvez seja necessário fazer Planos distintos Ação de Saúde Bucal. Estes PA-SB podem ser
                        ...
                        <Box display={"flex"} flexDirection={"column"} gap={5} mt={5}>
                            ...
                            {stepValues.thirdStep.causas.map((causa, index) => (
                                <Box key={index} pl={2} display={"flex"} flexDirection={"column"} gap={2}>
                                    <Typography fontWeight={700} fontSize={16}>
                                        Causa: {causa.causa}
                                    </Typography>
                    ...
                    <Paper sx={{ mt: 5, p: 4 }}>
                        ...
                        <Typography fontWeight={400} fontSize={16}>
                            O Plano de Ação expressa o que dever ser feito para resolução da situação problema. Descrever-se-á as ações que levarão à
                            modificação positiva do problema.
                        </Typography>
                        <Typography fontWeight={400} fontSize={16}>
                            <b>LEMBRE-SE!</b> Você pode consultar a Carta de recomendações gerada e disponibilizada pelo GestBucalSD após as
                            avaliações. E ainda, consultar o acervo da plataforma que contém produção técnica e científica para propor as ações ao
                            plano.
                        </Typography>
                        <Typography fontWeight={400} fontSize={16}>
                            Para cada ação definida, <b>deve-se ter:</b> a identificação de responsáveis (atores sociais), o levantamento dos recursos
                            necessários (material/tecnológico, financeiro, pessoal, etc.) e estabelecimento <b>prazo para execução do plano</b>.
                        </Typography>
                        <Typography fontWeight={400} fontSize={16}>
                            Como o ideal nem sempre é o real, pode-se antever estrategicamente algumas situações para se garantir maior viabilidade ao
                            plano.
                        </Typography>
                        <Typography fontWeight={400} fontSize={16}>
                            Estrategicamente, para cada ação definida, será analisada a motivação/interesse dos atores sociais e a viabilidade para os
                            recursos considerados críticos.
                        </Typography>
                        <Typography fontWeight={400} fontSize={16}>
                            Um ator social pode ser, apoiador, indiferente ou opositor as ações/plano, então quando o ator for considerado indiferente
                            ou opositor, a equipe deve descrever estratégias com o objetivo de eliminar resistências e buscar o apoio para a resolução
                            do problema, apoio ao plano. Isso vale também para a análise sobre os recursos necessários. Para aqueles considerados
                            críticos, também se descreverá as soluções para se obtê-los ou contorná-los.
                        </Typography>
                        <Typography fontWeight={700} fontSize={16}>
                            O plano de ação pode ser descrito à intervenção sobre vários problemas simultaneamente, então caso se tenha identificado
                            outros problemas/nós críticos, siga até esgotar a descrição das ações.
                        </Typography>

                        <Box mt={3} display={"flex"} flexDirection={"column"} gap={2}>
                            <Typography fontWeight={500} fontSize={16}>
                                Link do arquivo(Mapa mental): <b>{stepValues.fourthStep.mentalMapUrl}</b>
                            </Typography>
                        </Box>

                        <Box mt={3} display={"flex"} flexDirection={"column"} gap={2}>
                            <Typography fontWeight={500} fontSize={16}>
                                Digite o Nó crítico identificado: <b>{stepValues.fourthStep.criticalNode}</b>
                            </Typography>
                        </Box>
                    </Paper>
                            ...
                            <Paper key={index}>
                                <Box p={4} display={"flex"} flexDirection={"column"} gap={2}>
                                    ...
                                    {item.responsibles.map((responsible, responsibleIndex) => (
                                        <Box key={responsibleIndex} pl={2} display={"flex"} flexDirection={"column"} gap={2}>
                                            <Typography fontWeight={500} fontSize={16}>
                                                Responsável {responsibleIndex + 1}...n para ação {item.name}: <b>{responsible.responsible}</b>
                                            </Typography>
                                            <Typography fontWeight={500} fontSize={16}>
                                    ...
                                    {item.resources.map((resource, resourceIndex) => (
                                        <Box key={resourceIndex} pl={2} display={"flex"} flexDirection={"column"} gap={2}>
                                            <Typography fontWeight={500} fontSize={16}>
                                                Recurso {resourceIndex + 1}...n para ação {index + 1}: <b>{resource.resource}</b>
                                            </Typography>
                                            <Typography fontWeight={500} fontSize={16}>
                                                Este recurso é crítico?: <b>{resource.itsCricticalResource}</b>
                                            </Typography>
                                            <Typography fontWeight={500} fontSize={16}>
                                                DESCREVA AS ESTRATÉGIAS: <b>{resource.described_strategies}</b>
                                            </Typography>
                                    ...
                                    <Typography fontWeight={500} fontSize={16}>
                                        Prazo para cumprimento da Ação {index + 1}: <b>{item.deadline_compliance}</b>
                                    </Typography>
                                </Box>
                            </Paper>
                        ))}
                        ...
                        <Box display={"flex"} flexDirection={"column"} gap={5} mt={5}>
                            <Paper>
                                <Box p={4} display={"flex"} flexDirection={"column"} gap={2}>
                                    ...
                                    <Box width={"100%"}>
                                        <InputLabel id="health_establishment">Estabelecimento de Saúde</InputLabel>
                                        <TextField
                                            fullWidth
                                    ...
                                    <Box width={"100%"}>
                                        <InputLabel id="health_establishment">Município</InputLabel>
                                        <TextField
                                            fullWidth
                                            value={values.city}
                                            onChange={(e) =>
                    ...
                    {error && (
                        <Alert icon={<ErrorOutlineOutlined fontSize="inherit" />} severity="error">
                            Não foi possível enviar o formulário. Tente novamente mais tarde.
                        </Alert>
                    )}
                    <Box display="flex" alignItems="center" justifyContent={"flex-end"} gap={4} mt={10}>
                        <Button onClick={onClickPrevStep} variant="outlined">
                            Voltar
                        </Button>

                        <Button onClick={downloadPDF} variant="contained" color="primary" type={"submit"}>
                            {"Baixar PDF"}
                        </Button>
