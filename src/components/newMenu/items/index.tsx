import { Avatar, Box, Button, IconButton, Menu, MenuItem, Toolbar, useMediaQuery } from "@mui/material";
    ...
    return (
        ...
        >
            {!haveLogin ? (
                ...
                <IconButton size="large" edge="start" color="inherit" aria-label="menu" sx={{ mr: 2 }} onClick={() => setIsDrawerMenuOpen(true)}>
                    <MenuIcon />
                </IconButton>
            )}
            ...
            >
                {largeQuery ? itemsMenu.map((item, index) => <SimpleMenu key={index} element={item} />) : <></>}

                {!haveLogin ? <LoginModal /> : <LogoutButton />}

                {!largeQuery ? <MenuMobile /> : <></>}
